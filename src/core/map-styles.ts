...
export function snapSpacing(px: number, tolerance = SNAP_TOLERANCE_PX): string | null {
...
export function spacingClass(prefix: string, px: number, tolerance = SNAP_TOLERANCE_PX): string {
...
export function mapClasses(node: IRNode, tolerance = SNAP_TOLERANCE_PX): string[] {
  ...
  if (node.layout) {
    ...
    if (top === right && right === bottom && bottom === left) {
      ...
      if (top === bottom && top > 0) classes.push(spacingClass('py', top, tolerance))
      else {
        if (top > 0) classes.push(spacingClass('pt', top, tolerance))
        if (bottom > 0) classes.push(spacingClass('pb', bottom, tolerance))
      }
      if (left === right && left > 0) classes.push(spacingClass('px', left, tolerance))
      else {
  ...
  if (node.style.cornerRadius > 0) {
    ...
    classes.push(step !== null ? `rounded-[${Math.round(node.style.cornerRadius)}px]` : `rounded-[${Math.round(node.style.cornerRadius)}px]`)
  }
  ...
  if (node.text) {
    ...
    if (node.text.typography.color) classes.push(`text-[${node.text.typography.color}]`)
    if (node.text.typography.fontWeight >= 700) classes.push('font-bold')
    else if (node.text.typography.fontWeight >= 500) classes.push('font-medium')
  }
